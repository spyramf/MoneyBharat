  ...
  useEffect(() => {
    ...
    const addSecurityOptimizations = () => {
      ...
      const securityTags = [
        ...
        { httpEquiv: 'Content-Security-Policy', content: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://www.googletagmanager.com https://www.google-analytics.com https://cdn.gpteng.co; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https://www.google-analytics.com https://analytics.google.com" }
      ];
    ...
    const addAdvancedStructuredData = () => {
      ...
      script.textContent = JSON.stringify({
        ...
        serviceType: ['Mutual Fund Investment', 'SIP Investment', 'Health Insurance', 'Term Life Insurance', 'Personal Loans', 'Home Loans', 'Business Loans'],
        areaServed: 'IN',
    ...
    const addAdditionalSEOMeta = () => {
      ...
      seoTags.forEach(tag => {
        const existing = document.querySelector(`meta[name="${tag.name}"]`);
        if (!existing && tag.content !== 'your-google-verification-code' && tag.content !== 'your-bing-verification-code' && tag.content !== 'your-yandex-verification-code') {
          const meta = document.createElement('meta');
          meta.name = tag.name;
          meta.content = tag.content;
          document.head.appendChild(meta);
