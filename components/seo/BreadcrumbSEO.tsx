...
const BreadcrumbSEO = () => {
  ...
  const structuredData = {
    ...
    itemListElement: breadcrumbs.map((item, index) => ({
      ...
      position: index + 1,
      name: item.name,
      item: `${process.env.NEXT_PUBLIC_SITE_URL || ''}${item.href}`, // Use environment variable for site URL
    })),
  };
            ...
            {breadcrumbs.map((item, index) => (
              <li key={item.href} className="flex items-center">
                ...
                {index === breadcrumbs.length - 1 ? (
                  ...
                  <Link href={item.href} className="text-fintech-green hover:text-fintech-green/80 transition-colors">
                    {item.name}
                  </Link>
                )}
              </li>
